...
*
**/
public class SelectVoList extends AbstractMethod {

    private final String method = "selectVoList";
    private final String desc = "查询满足条件所有数据";
    private final String sqlVal = "<script>%s SELECT %s FROM %s %s %s\n</script>";

    @Override
    public MappedStatement injectMappedStatement(Class<?> mapperClass, Class<?> modelClass, TableInfo tableInfo) {
        String sql = String.format(sqlVal, sqlFirst(), sqlSelectColumns(tableInfo, true), tableInfo.getTableName(),
